...
lock = asyncio.Lock()
current_order = None
current_uid = None  # Track current UID for SMM Panel
...
async def smm_share(session, cookie, token, post_link, account_uid, display_mode='detailed', target_shares=None):
        ...
        try:
            ...
            async with session.post(url, headers=headers, timeout=10) as response:
                ...
                if 'id' in data:
                    ...
                    if display_mode != 'minimal':
                        print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(error_msg[:50], 'red_orange')}")
                    break
            ...
            if display_mode != 'minimal':
                print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(str(e)[:50], 'red_orange')}")
            break
...
async def smm_boost_main(cookie, post_link, share_count, account_uid, order_info=None):
    """SMM Panel Boost using content_management method"""
    global success_count
    success_count = 0
    
    async with aiohttp.ClientSession() as session:
        print(f" {G('[!] EXTRACTING EAAG TOKEN (CONTENT_MANAGEMENT)...')}")
        nice_loader("EXTRACTING")
        
        token = await smm_get_token(session, cookie)
        
        if not token:
            print(f" {R('[ERROR] Failed to extract EAAG token!')}")
            print(f" {C('Cookie may be invalid or expired.')}")
            return False
        
        print(f" {G('[SUCCESS] Token extracted!')}")
        print(LINE)
        ...
        print(f" {C('Cookie UID:')} {G(account_uid)}")
        ...
        await smm_share(session, cookie, token, post_link, account_uid, display_mode, share_count)
...
def generate_order_receipt(order_info, shares_completed, start_time, end_time, account_uid=None):
    ...
    print(f" {W}[{RESET}•{W}]{RESET} {C('SHARES COMPLETED')}  {W}➤{RESET} {G(str(shares_completed))}")
    ...
    if account_uid:
        print(f" {W}[{RESET}•{W}]{RESET} {C('COOKIE UID')}        {W}➤{RESET} {G(account_uid)}")
...
def start_smm_panel_boost():
    ...
    global success_count, current_uid
    ...
    print(f" {C('• Uses single cookie')}")
    print(LINE)
    print(f" {Y('[!] SUPPORTED POST LINK FORMAT:')}")
    ...
    if not cookies:
        print(f" {R('[ERROR] No cookies stored in database')}")
        print(f" {C('[TIP] Add cookies first using Manage Cookies')}")
        input(f"\n {G('[PRESS ENTER TO CONTINUE]')}")
        return
    
    refresh_screen()
    print(f" {M('[SELECT COOKIE FOR SMM BOOST]')}")
    print(LINE)
    ...
    cookie_choice = input(f" {W}[{W}➤{W}]{RESET} {C('SELECT COOKIE NUMBER')} {W}➤{RESET} ").strip()
    
    if not cookie_choice or not cookie_choice.isdigit():
        return
    ...
    selected_cookie = cookies[cookie_index]
    current_uid = selected_cookie['uid']  # Store UID for display
    ...
    if not validate_smm_post_link(post_link):
        ...
        print(LINE)
        print(f" {R('⚠ The link you provided is not a valid Facebook share link.')}")
        print(LINE)
        print(f" {Y('[!] SUPPORTED FORMATS:')}")
        print(f" {C('• https://www.facebook.com/share/p/XXXXX/')}")
        print(f" {C('• https://www.facebook.com/share/XXXXX/')}")
        print(LINE)
    ...
    try:
        asyncio.run(smm_boost_main(selected_cookie['cookie'], post_link, share_count, selected_cookie['uid'], order_info))
        
        end_time = datetime.datetime.now()
        
        # Generate receipt with banner
        generate_order_receipt(order_info, success_count, start_time, end_time, selected_cookie['uid'])
