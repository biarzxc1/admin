    ...
    while True:
        ...
        try:
            ...
            if '"registration_succeeded":true' in c.text:
                ...
                cookie_dict,cookie_str_temp=parse_set_cookie(c.headers)
                ...
                if not ids:
                    ...
                    if uid_match:
                        ...
                        uid_match = re.search(r'"userid":"(\d+)"', c.text)
                        if uid_match:
                            ids = uid_match.group(1)
                        else:
                            uid_match = re.search(r'"id":(\d+)', c.text)
                            if uid_match:
                                ids = uid_match.group(1)
                
                # If still no ID, try logging in to get full cookies
                if not ids or ids == "Unknown":
                    try:
                        ...
                        login_response = requests.post('https://www.facebook.com/login.php', 
                                                      data=login_data, 
                        ...
                        ids = login_cookies.get("c_user", ids)
                        
                        # Update cookies with login cookies
                        cookie_dict.update(login_cookies)
                    except:
                        pass
                
                # Build complete cookie string
                all_cookies = {}
                all_cookies.update(cookies)  # Initial cookies
                all_cookies.update(first_cok)  # Registration cookies
                all_cookies.update(cookie_dict)  # Parsed cookies
                
                # Add c_user if we found it
                if ids and ids != "Unknown":
                    all_cookies['c_user'] = ids
                    all_cookies['xs'] = all_cookies.get('xs', 'null')
                
                # Create full cookie string
                cookie_str = ";".join(f"{k}={v}" for k, v in all_cookies.items() if v)
                
                # Final fallback for ID
                if not ids or ids == "Unknown":
                    ids = f"FB_{random.randint(100000000000, 999999999999)}"
