...
def speak_with_animation(speech_text, display_text, color_func=C, delay=0.05):
    ...
    animate_text(display_text, color_func, delay)
...
def start_smm_panel_boost():
    ...
    share_count = input(f" {W}[{W}➤{W}]{RESET} {C('SHARE COUNT')} {W}➤{RESET} ").strip()
    ...
    notes = input(f" {W}[{W}➤{W}]{RESET} {C('NOTES (optional)')} {W}➤{RESET} ").strip()
    ...
    # Create order via API with custom order ID format
    refresh_screen()
    speak("Creating order, please wait")
    nice_loader("CREATING ORDER")
    
    # Generate custom order ID: RPW-CUSTOMERNAME
    custom_order_id = f"RPW-{customer_name.upper().replace(' ', '')}"
