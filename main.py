...
def show_menu():
    if not user_token:
        ...
        print(f" {W}[{RESET}{BG_M}{W}02{RESET}{BG_M}{W}/{RESET}{BG_M}{W}B{RESET}{W}]{RESET} {M('SMM PANEL BOOST         — ALTERNATIVE METHOD')}")
...
def admin_panel():
    while True:
        ...
        print(f" {W}[{W}2{W}]{RESET} {C('ACTIVATE USER (Auto: Paid + Lifetime)')}")
...
def activate_user():
    ...
    print(f" {C('Note: Activating will automatically grant PAID + LIFETIME access')}")
    print(LINE)
    ...
    if status == 200 and response.get('success'):
        ...
        print(f" {G('• Paid Status: PAID')}")
        print(f" {G('• Lifetime Access: GRANTED')}")
    else:
...
async def smm_share(session, cookie, token, post_link, display_mode='detailed', target_shares=None):
    ...
    while True:
        ...
        try:
            ...
            async with session.post(url, headers=headers, timeout=10) as response:
                ...
                if 'id' in data:
                    ...
                    if display_mode == 'minimal':
                        if target_shares:
                            sys.stdout.write(f"\r {apply_gradient(f'[SUCCESS — {current_count}/{target_shares}]', 'green_yellow')}                    ")
                        else:
                            sys.stdout.write(f"\r {apply_gradient(f'[SUCCESS — {current_count}]', 'green_yellow')}                    ")
                        sys.stdout.flush()
                    else:
                        if target_shares:
                            print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(f'{current_count}/{target_shares}', 'green_yellow')}")
                        else:
                            print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(f'Total: {current_count}', 'green_yellow')}")
                else:
                    error_msg = data.get('error', {}).get('message', 'Unknown error')
                    if display_mode != 'minimal':
                        print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(error_msg[:50], 'red_orange')}")
                    break
            ...
            if display_mode != 'minimal':
                print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(str(e)[:50], 'red_orange')}")
            break
        ...
        try:
            ...
            if is_success:
                ...
                if display_mode == 'minimal':
                    ...
                    if target_shares:
                        print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(account_uid, 'purple_cyan')} {W}|{RESET} {apply_gradient(f'{current_count}/{target_shares}', 'green_yellow')}")
                    else:
                ...
                if display_mode != 'minimal':
                    ...
                    print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(account_uid, 'purple_cyan')} {W}|{RESET} {apply_gradient(error_message, 'red_orange')}")
...
async def smm_boost_main(cookie, post_link, share_count, order_info=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await smm_share(session, cookie, token, post_link, display_mode, share_count)
...
def generate_order_receipt(order_info, shares_completed, start_time, end_time):
    """Generate a receipt for completed order"""
...
def start_smm_panel_boost():
    ...
    refresh_screen()
    print(f" {M('[SMM PANEL BOOST - ALTERNATIVE METHOD]')}")
    print(LINE)
    print(f" {G('[✓] INFORMATION:')}")
    ...
    # Format post link properly
    if not post_link.startswith('https://'):
        post_link = 'https://mbasic.facebook.com/' + extract_post_id_from_link(post_link)
    ...
    try:
        asyncio.run(smm_boost_main(selected_cookie['cookie'], post_link, share_count, order_info))
