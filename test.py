...
def validate_smm_post_link(url):
    ...
    - https://facebook.com/share/p/XXXX/
    - https://facebook.com/share/XXXX/
    - https://m.facebook.com/share/p/XXXX/
    - https://m.facebook.com/share/XXXX/
    """
    if not url:
        return False
    ...
    # Pattern for valid Facebook share links
    patterns = [
...
async def smm_share(session, cookie, token, post_link, account_name, display_mode='detailed', target_shares=None):
    ...
    while True:
        ...
        try:
            ...
            async with session.post(url, headers=headers, timeout=10) as response:
                ...
                if 'id' in data:
                    ...
                    if display_mode == 'minimal':
                        if target_shares:
                            sys.stdout.write(f"\r {apply_gradient(f'[SUCCESS — {current_count}/{target_shares}]', 'green_yellow')} {W}|{RESET} {apply_gradient(f'[{account_name}]', 'purple_cyan')}                    ")
                        else:
                            sys.stdout.write(f"\r {apply_gradient(f'[SUCCESS — {current_count}]', 'green_yellow')} {W}|{RESET} {apply_gradient(f'[{account_name}]', 'purple_cyan')}                    ")
                        sys.stdout.flush()
                    else:
                        if target_shares:
                            print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(account_name, 'purple_cyan')} {W}|{RESET} {apply_gradient(f'{current_count}/{target_shares}', 'green_yellow')}")
                        else:
                            print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(account_name, 'purple_cyan')} {W}|{RESET} {apply_gradient(f'Total: {current_count}', 'green_yellow')}")
                else:
                    error_msg = data.get('error', {}).get('message', 'Unknown error')
                    if display_mode != 'minimal':
                        print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(account_name, 'purple_cyan')} {W}|{RESET} {apply_gradient(error_msg[:50], 'red_orange')}")
                    break
            ...
            if display_mode != 'minimal':
                print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(account_name, 'purple_cyan')} {W}|{RESET} {apply_gradient(str(e)[:50], 'red_orange')}")
            break
    ...
    while True:
        ...
        try:
            ...
            if is_success:
                ...
                if display_mode == 'minimal':
                    if target_shares:
                        sys.stdout.write(f"\r {apply_gradient(f'[SUCCESS — {current_count}/{target_shares}]', 'green_yellow')} {W}|{RESET} {apply_gradient(f'[UID: {account_uid}]', 'purple_cyan')}                    ")
                    else:
                    ...
                    if target_shares:
                        print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(f'{current_count}/{target_shares}', 'green_yellow')}")
                    else:
                        print(f" {apply_gradient('[SUCCESS]', 'green_yellow')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(f'Total: {current_count}', 'green_yellow')}")
                
            else:
                if display_mode != 'minimal':
                    error_message = result[:40]
                    print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(current_time, 'blue_purple')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(error_message, 'red_orange')}")
            ...
            if "asyncio" not in error_msg.lower() and "event" not in error_msg.lower():
                if display_mode != 'minimal':
                    print(f" {apply_gradient('[ERROR]', 'red_orange')} {W}|{RESET} {apply_gradient(datetime.datetime.now().strftime('%H:%M:%S'), 'blue_purple')} {W}|{RESET} {apply_gradient(f'UID: {account_uid}', 'purple_cyan')} {W}|{RESET} {apply_gradient(error_msg[:40], 'red_orange')}")
...
async def auto_share_main(link_or_id, selected_cookies, order_info=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(LINE)
        print(f" {C('Mode:')} {G('NORMAL (BUSINESS_LOCATIONS)')}")
...
async def smm_boost_main(selected_cookies, post_link, share_count, order_info=None):
    """SMM Panel Boost using content_management method with multiple cookies"""
    global success_count
    success_count = 0
    
    async with aiohttp.ClientSession() as session:
        print(f" {G('[!] EXTRACTING EAAG TOKENS (CONTENT_MANAGEMENT)...')}")
        nice_loader("EXTRACTING")
        ...
        for cookie_data in selected_cookies:
            ...
            if token:
                ...
                print(f" {R('✗')} {C(cookie_data['name'])} {R('Failed to extract token')}")
        ...
        print(LINE)
        print(f" {G(f'[SUCCESS] Extracted {len(valid_tokens)} token(s)!')}")
        print(LINE)
        ...
        print(LINE)
        print(f" {C('Method:')} {M('CONTENT_MANAGEMENT')}")
...
def generate_order_receipt(order_info, shares_completed, start_time, end_time):
    ...
    if hours > 0:
        duration_str = f"{hours}Hours {minutes}Minutes {seconds}Sec"
    elif minutes > 0:
        duration_str = f"{minutes}Minutes {seconds}Sec"
    else:
    ...
    print(f" {W}[{RESET}•{W}]{RESET} {C('DURATION')}          {W}➤{RESET} {M(duration_str)}")
    print(LINE)
...
def start_auto_share():
    ...
    try:
        asyncio.run(auto_share_main(link_or_id, selected_cookies, None))
...
def start_smm_panel_boost():
    ...
    print(LINE)
    print(f" {G('[✓] INFORMATION:')}")
    print(f" {C('• Uses content_management method')}")
    print(f" {C('• NO DELAYS - Instant sharing')}")
    print(f" {C('• Creates order for tracking')}")
    print(f" {C('• Supports multiple cookies')}")
    print(LINE)
    print(f" {Y('[!] SUPPORTED POST LINK FORMATS:')}")
    print(f" {C('• https://www.facebook.com/share/p/XXXXX/')}")
    print(f" {C('• https://www.facebook.com/share/XXXXX/')}")
    print(LINE)
    print(f" {R('[!] NOT SUPPORTED FORMATS:')}")
    print(f" {R('Any other Facebook link formats are NOT supported!')}")
    print(LINE)
    ...
    # Get order details
    refresh_screen()
    print(f" {M('[CREATE SMM BOOST ORDER]')}")
    print(LINE)
    ...
    if not validate_smm_post_link(post_link):
        refresh_screen()
        print(f" {R('[ERROR] Post Link NOT SUPPORTED!')}")
        print(LINE)
        print(f" {R('⚠ The link you provided is NOT a valid share link.')}")
        print(LINE)
        print(f" {Y('[!] SUPPORTED FORMATS ONLY:')}")
        print(f" {C('• https://www.facebook.com/share/p/XXXXX/')}")
        print(f" {C('• https://www.facebook.com/share/XXXXX/')}")
        print(LINE)
        print(f" {R('All other link formats are NOT SUPPORTED.')}")
        print(f" {C('Please contact the admin for guide on how to get the correct link.')}")
        print(LINE)
        input(f"\n {G('[PRESS ENTER TO CONTINUE]')}")
        return
    ...
    try:
        asyncio.run(smm_boost_main(selected_cookies, post_link, share_count, order_info))
